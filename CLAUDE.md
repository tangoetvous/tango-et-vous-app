...
## Session 2026-09-04 — Emails jamais envoyés depuis les formulaires ouverts dans l'admin (✅ FAIT — VÉRIFIÉ EN RÉEL)

Bug réel : « Inscriptions Tango → ✏️ Nouvelle demande » enregistrait bien l'inscription mais **n'envoyait ni email élève, ni email admin, ni push**. Invisible depuis le site Wix.

- **Cause** : ce bouton (et 4 autres) ouvre le **formulaire public dans une iframe** (`admin.html` ~L15308, `inscription-cours.html?admin=1`). Le formulaire enregistrait (`await`), puis **lançait sans l'attendre** la requête `/api/notify/…`, puis prévenait l'admin par BroadcastChannel. L'admin réagit en changeant de sous-onglet → **l'iframe est retirée du DOM → le navigateur ANNULE la requête en vol**. Depuis Wix, la page parente ne détruit rien : la requête aboutit, d'où un bug visible uniquement côté admin.
- **Les 5 formulaires ouverts en iframe depuis l'admin étaient touchés** : `inscription-cours` (Nouvelle demande + Pré-inscription), `cours-essai` (Essai Tango → Inscrire), `stages-pwa` (Stages → Inscrire), `essai-yoga` (Yoga → Essai yoga → Inscrire), `cours-particuliers` (onglet Cours particuliers). **`stages-pwa` était le pire** : il prévenait l'admin AVANT même de lancer l'envoi. 📌 `demande-devis.html` n'est PAS concerné (son envoi est déjà `await` — c'est l'enregistrement lui-même).
- **Fix** : helper **`_tevNotify(url, data)`** dupliqué dans les 5 formulaires — `keepalive: true` (le navigateur mène la requête à terme malgré la fermeture) **+ attente bornée à 4 s** (`Promise.race`), jamais bloquante. Et **l'envoi précède toujours la notification de l'admin** (ordre inversé dans `stages-pwa`).
- 📌 **RÈGLE : dans un formulaire ouvert en iframe depuis l'admin, une notification ne doit JAMAIS être « lancée et oubliée ».** Étend la règle existante (« INSERT avant BroadcastChannel ») : l'envoi des emails est soumis à la même contrainte que l'INSERT.
- **Bouton « ✉️ Renvoyer l'email »** (`renvoyerEmailInscription`, fiche dépliée d'Inscriptions Tango, à côté du ✕) : rejoue I01 pour UNE ligne, charge utile reconstruite depuis la fiche (variante validée/attente déduite du statut, `quotaFull` à null → repli historique), confirmation nommant les destinataires. Un cours par envoi : une personne inscrite à 2 cours demande 2 envois. 📌 Aucun marqueur ne distingue en base une saisie admin d'une inscription Wix — pour retrouver les fiches restées sans email : **une inscription sans notification correspondante dans le panel 🔔 n'a rien reçu**.
- 🐛 **Régression introduite puis corrigée le même jour** : l'attente de l'envoi décale la notification de quelques secondes → le rechargement (polling 15 s / realtime) a le temps de charger la ligne réelle → `_appliquerCoursInscription` poussait une **2ᵉ copie** (doublon à l'écran, une seule ligne en base). Fix : **`_ctDejaLa`** rend l'ajout optimiste idempotent (même cours + même saison ; match par email, sinon par nom normalisé pour les fiches partenaire sans adresse), posé sur les 4 ajouts (inscripteur/partenaire × cours 1/cours 2). La base reste seule juge — `chargerDonnees` reconstruit la liste ensuite, **vrais doublons d'inscription compris** (la politique « aucun filtre anti-doublon » est préservée). Le défaut était latent depuis toujours : le realtime pouvait déjà gagner la course.
- **Tests groupe AI** (`tests/ai-notify-iframe.spec.js`, 6) : AI1 les 5 formulaires ont `_tevNotify` + `keepalive` et plus aucun `fetch(notify)` nu ; AI2 l'envoi précède toujours le BroadcastChannel (**c'est la cause même du bug qui est verrouillée**) ; AI3/AI4 charge utile du renvoi (solo en attente, couple validé) ; AI5 garde-fous (sans email, mode démo) ; AI6 anti-doublon (4 situations).
- ✅ **Bouton « ✉️ Renvoyer l'email » vérifié en réel (2026-09-04)** : l'email élève part bien. Réserve signalée par l'admin : il déclenche aussi le push et l'email admin « nouvelle demande » — inutiles pour un renvoi, à retirer (cf. todolist).
- ✅ **Vérifié en réel par l'admin (2026-09-04)** : les 4 formulaires publics depuis **Wix** (inscription, essai tango, stages, essai yoga) ; le bouton **Nouvelle demande** côté admin (email reçu) ; et l'absence de doublon après le correctif. Restent à essayer : les 4 autres boutons admin et `cours-particuliers` depuis Wix (modifié après la série de tests).

## Session 2026-09-04 — Adresse : champ « Note d'accès » saisissable (✅ FAIT)

Demande admin : afficher sous l'adresse des emails de cours d'essai une consigne d'entrée (digicode du portail de la villa Riberolle), **pour Paris uniquement**.

- **Le worker savait déjà rendre `adresse.note`** (E0/E1/E2/E5/E5b/E6, E4, E-mod, I01) mais **le formulaire des Paramètres n'avait aucun champ pour la saisir** : champ lu, jamais écrit. Fix : champ **« Note d'accès (facultatif) »** dans Paramètres → &lt;lieu&gt; → Adresse (`_renderSecAdresseContent` + `sauverAdresseType`), disponible pour Paris / Vincennes / Yoga / Stages — **chacun sa note, aucune fuite d'une ville à l'autre** (le cloisonnement est structurel : la note vit dans `tev_params_<ville>_<sai>.adresse`).
- **E15** (validation d'une personne en attente) était le seul email d'essai à ignorer la note → aligné sur E4. **Volontairement exclus** : les emails d'annulation et de report (la personne ne vient pas).
- 📌 **Périmètre réel** (vérifié, une première annonce était fausse) : côté cours réguliers, **seul I01** affiche la note — **I02, I03 et la relance du 22 août ne relisent pas ce champ**.
- **Rendu** : **rouge `#c62828` semi-gras** (demande admin — en gris italique ça faisait un pâté), **entre la rue et le transport**. Italique retiré : rouge + italique + petite taille = illisible.
- **Tests groupe AH** (`tests/ah-adresse-note.spec.js`, 4) : champ présent pour les 4 lieux sans toucher aux champs existants ; aller-retour saisie/relecture ; cloisonnement Paris vs Vincennes ; note vide. Preview essai régénérée depuis le code + page des sources à jour.
...
- **Cause** : `_buildCartesData` ne retient une carte portée par `eleves` que si `eleves.saison === saison affichée`. Or **les 2 chemins de réinscription laissent volontairement la fiche eleves intacte** — inscription directe (garde `_elvAutreSaison`, admin.html ~L15494, dont le commentaire dit déjà « bug Vlad 2026-07-08 ») et Valider le paiement (upsert `ignoreDuplicates:true`, ~L15181). La fiche d'un réinscrit reste donc estampillée saison précédente → aucune carte eleves dans la vue courante → **pseudo-carte** `_fromCoursTango` portant l'**id `inscriptions_cours`** → `tevPointerCours` cherche cet id dans `eleves` → introuvable, à 100 %.
- **Le correctif du 2026-08-18 ne couvrait que la moitié du problème** : sa condition d'entrée exigeait `isReport && saisonOrigine`. Les cartes reportées (pseudo-cartes elles aussi) étaient réparées ; les pseudo-cartes de réinscrits, jumelles, ne l'étaient pas. **Règle : tout traitement d'une pseudo-carte doit viser `_fromCoursTango`, pas `isReport`.**
- **Fix** (`pointerCoursAction`, admin.html) : `_isRepPtr` devient `!!(c && c._fromCoursTango)` — le chemin de démarrage déjà éprouvé (résolution du vrai `eleves.id` par email, écriture compteurs/date d'achat/expiration/`saison=saisonActive()`, présences insérées, CP-E + C1) sert désormais aux deux cas. `_vraiRepPtr` (`isReport && saisonOrigine`) distingue ce qui reste spécifique au report : expiration d'origine héritée, `_dejaRep=0`, `datesCours` remises à zéro.
- **Reprise des cours déjà saisis** : une pseudo-carte pointée via ✏️ Modifier stocke ses cours dans `donnees` et **jamais dans `presences`** → au démarrage, `carte_utilises = donnees.utilises + n`, `carte_date_achat` = 1ʳᵉ date conservée, et les dates antérieures sont insérées dans `presences` (note « Cours saisi avant reprise ») pour que l'historique de l'espace élève soit complet. C1 n'est envoyé que si la carte démarre vraiment (`_dejaRep===0`).
...
La session « App T&V » (ouverte le 2026-04-05, celle qui a construit l'essentiel de l'app) est morte le 04/09 à 08:31 Paris sur `ref_not_found` / `recoverable:false`. **Cause** : elle avait été créée avec `revision: refs/heads/main`, or `main` n'existe plus sur GitHub (seules subsistent 3 branches `claude/*`, la branche par défaut étant `claude/dance-school-app-RTqb5`). Tant que son conteneur était restauré à chaud, la référence n'était jamais relue ; la première **reconstruction à froid** (mise à jour de plateforme : CLI 2.1.233 → 2.1.260) a déclenché un reclone… sur une branche disparue.

- **Pourquoi `main` a disparu — hypothèse forte de l'admin (2026-09-04)** : la suppression fait très probablement partie de la **purge RGPD de l'audit du 2026-05-06** (point C1 : « Backups CSV retirés du repo git » — fichiers nominatifs). GitHub interdisant de supprimer la branche par défaut, il fallait d'abord basculer le défaut sur `claude/dance-school-app-RTqb5` puis supprimer `main` : cela explique à la fois le choix inhabituel d'une branche `claude/*` comme branche par défaut et l'absence de trace « changement de branche » (le geste était vécu comme une mesure de sécurité, consignée sous « Audit sécurité »). **Chronologie compatible** : `main` existait le 05/04 (la session d'avril porte `refs/heads/main` et a démarré normalement) ; les sessions de juin/juillet portent déjà `refs/heads/claude/dance-school-app-RTqb5` → basculement entre avril et juin, l'audit est du 6 mai. Non prouvé formellement (le clone de session est superficiel, rien avant le 17 août).
- 📌 **Règle** : avant de supprimer une branche, vérifier qu'aucune session Claude n'y est adossée (une session créée avec `revision: refs/heads/<branche>` meurt à sa première reconstruction à froid) — et consigner ici tout changement de branche par défaut, **y compris quand il découle d'une mesure de sécurité** : c'est justement le cas où l'on n'y pense pas.
- ⛔ **NE PAS recréer `main`** (décision admin 2026-09-04). Sa suppression est très probablement une mesure RGPD délibérée — ne pas la défaire pour le confort d'une session. Recréer la branche sur le commit courant ne restaurerait d'ailleurs rien (un nouveau pointeur ne contient que l'historique actuel), mais brouillerait la lecture pour la suite. L'ancienne session reste donc définitivement close.
- **Rien n'a été perdu** : le travail était poussé, et la mémoire du projet vit dans ce fichier — c'est exactement ce à quoi il sert. La session perdue était de toute façon à 96 % de sa capacité de contexte.
- **Protections backups vérifiées le 2026-09-04** : `backups/` dans `.gitignore`, route `/backups/*` → 403 dans `worker.js` (~L62), workflow de sauvegarde en GitHub Artifacts uniquement, aucun fichier de backup dans l'historique accessible.
...
## Session 2026-08-28 — Cartes reportées : l'expiration d'ORIGINE est conservée (✅ FAIT — décision admin)

**Changement de règle métier validé par l'admin** (remplace « validité recalculée au 1er cours » de la maquette carte reportée du 2026-08-18) : une carte reportée **garde sa date d'expiration de la saison d'origine** — ces dates (ex. 1 oct, 13 nov) intègrent déjà la coupure d'été via calcExpiration, le report ne doit pas rallonger la validité. **Cartes NON reportées : strictement rien ne change** (validité au 1er cours pointé, comme avant — prouvé par AD3 octet pour octet et AE2/AE4).

- **Champ séparé `reportedExpiration`** (jamais `expiration` sur la pseudo-carte → aucun mécanisme existant touché : pas de badge « Expirée », boutons intacts). Sources en cascade : `donnees.reportedExpiration` (posé au report par `reporterCarteJs` ET `validerReport` désormais) → sinon **retrouvé rétroactivement** sur la fiche eleves de la saison d'origine (email + nom normalisé) — les cartes déjà reportées avant ce correctif affichent donc leur date sans rien refaire.
- **Admin** : fiche Détails « Report de X · Exp. [date] » (repli « Dém. au 1er cours » si date introuvable) ; modale ✏️ préremplit l'expiration héritée (collante à la sauvegarde) ; `sauvegarderEditCarte` (vrai report) préserve `reportedExpiration` dans donnees.
- **Redémarrage « + Pointer »** : `carte_expiration = date héritée` + **`carte_exp_manuelle=true`** (les pointages suivants ne recalculent jamais — mécanisme date forcée existant) ; sans date héritée → calcul auto historique (`carte_exp_manuelle=false`). Le renouvellement suivant remet le calcul auto (comportement standard).
...
📌 **Même session — bascule du 1er septembre : 3 groupes de tests gelés dans le temps.** La suite lancée le 01/09 a révélé que les groupes **S** (fixtures essai datées 2026-06-11), **U** et **W** (stage fixture 2026-08-01) cassaient : leurs dates figées appartiennent à la saison 2025-2026, désormais filtrée par `dateAppartientSaison` (même mécanique que la panne du groupe T le 27/08). Fix identique : `page.clock.setFixedTime(ANCRE)` posé AVANT le `bootDemo` de chaque test (ancres en pleine saison 2025-2026, cohérentes avec les fixtures). **Règle confirmée : tout groupe à fixtures datées FIXES doit geler son horloge** — les groupes à dates relatives (AD, AF, K, R, X…) n'en ont pas besoin. ⚠️ Piège de session : un `| tail -1` sur la sortie Playwright avait masqué la ligne « 4 failed » au-dessus de « 7 passed » — toujours grepper `passed|failed` (les DEUX lignes).
...
- **Fiche admin** (`ouvrirContact`, ctx='essai') : encadré « 💬 Remarque du formulaire » si non vide — match par email, repli `_normNom` pour les fiches sans email ; plusieurs remarques (plusieurs dates) → une carte par remarque avec la date. **Rétroactif** : toutes les remarques déjà en base sont consultables. + **Pastille 💬** sur les lignes (vue Par date + accordéon Pointage), infobulle = le texte de la remarque (`escHtml` partout — champ public).
- **Email E0** : le formulaire transmet `remarque` dans la charge notify ; `handleNotifyInscriptionEssai` la destructure et remplit la ligne 💬 du tableau (variante confirmée) ou une ligne dans l'encadré du cours (variante attente, qui n'a pas de tableau). Vide → « — » / rien, rendu strictement inchangé. Preview essai régénérée (cas E0+E1 porte une remarque d'exemple) + page sources à jour.
- 📌 Cosmétique même session (formulaires publics, demande admin) : textes agrandis sur `inscription-cours.html` (3 étapes, cases centrées, FACULTATIF or) et `cours-essai.html` (étapes 1-2-4, flèches → espace dans les cases niveau, « Vous venez… » → « VOUS ÊTES… », jour du cours à la ligne, titres du récap en blanc, Facultatif en or).
...
- ✅ **SQL exécuté par l'admin (2026-08-27)** — quota ACTIF. Le comptage RPC a même révélé un doublon réel (24 vs 23 affichés : l'admin dédoublonne par nom, la base comptait 2 lignes) — supprimé par l'admin via l'interface. Testé en réel : guideur seul ET couple bien mis en attente.
- ✅ **Emails « cours complet » (2026-08-27, validés par l'admin)** : le gabarit I01-att unique servait aussi aux mises en attente par quota → un couple recevait « Vous vous inscrivez sans partenaire » et « Trouvez un·e partenaire » (absurde). Le formulaire transmet désormais la RAISON (`quotaFull1/2 = {gui, gde}` dans la charge utile notify), et `handleNotifyInscriptionCours` décline 3 variantes : **I01-att** (guidée seule, parité — STRICTEMENT inchangé), **I01-complet** (seul·e, quota — « Ce cours affiche actuellement complet pour les guideur·se·s/guidé·e·s », SANS « trouvez un·e partenaire »), **I01-quota-att** (duo, quota — « vous serez confirmé·e·s tous les deux ensemble », ligne « 👫 En duo avec X »). Options des 2 variantes quota (décision admin) : 1️⃣ rester en attente · 2️⃣ « choisir un autre cours de ce niveau (Paris et Vincennes) » → lien formulaire ; le cours d'essai RETIRÉ (frustrant si complet). Repli : couple+wait sans quotaFull (vieux cache) → variante duo déduite ; solo sans quotaFull → I01-att historique. Preview régénérée (13 cas / 29 emails) + page sources à jour ; AF1/AF2/AF4 vérifient la charge utile quotaFull.
...
- **Pages** : `preview-formulaires-clairs-v1.html` (index : bandeau « générée depuis le code », engagement, tables de correspondance dépliables) + dossier `preview-formulaires-clairs/` (3 pages, une par formulaire, étapes empilées). **Générées par `outils/generer-preview-formulaires-clairs.js`** : chaque formulaire RÉEL est rendu par Playwright (fixtures localStorage à dates relatives, Supabase mocké 406, zéro réseau), son DOM est capturé (scripts retirés), puis une **table de correspondance de couleurs** est appliquée — structure/textes/polices strictement intacts.
- **Palette cible = `cours-particuliers.html`** (référence validée) : fond `#faf8f4`, cartes blanches, **or foncé `#6e5100`** (boutons à texte blanc), or pâle `#f5f0e4` pour les sélections, **header conservé noir + or vif `#D4AF37`**. La conversion réelle, si validée, = appliquer la MÊME table aux fichiers sources (CSS + couleurs dans les gabarits JS), header exclu.
- **Mécanique du générateur** : règles CSS `.header/.hdr/.logo` masquées pendant la conversion (jetons ``-gardés — le logo base64 peut contenir n'importe quelle suite alphanumérique) ; règles composées AVANT la table (`color:#000|#111` sur fonds or → `#fff`, `background:#000` des barres de progression → clair) ; `rgba(255,255,255,α)` → noirs, halos or → `rgba(110,81,0,…)` ; **vérificateur de luminance** listant les fonds encore sombres / textes encore clairs restants (tous intentionnels au final : boutons or foncé, badges accent, bloc rouge de confirmation stages laissé auto-contrasté).
- ⚠️ **Pièges rencontrés** : (1) `inscription-cours` colore son header via `var(--gold)` → le remap du `:root` l'assombrissait malgré la protection des règles ; épinglé par un style `tev-header-identite` (la vraie conversion codera `#D4AF37` en dur dans `.hdr-logo`, comme cours-particuliers). (2) `cours-essai` ET `stages-pwa` répètent **67×** « ↑ Remonter… » (remplissage voulu pour l'iframe Wix) → tronqué à 1 occurrence dans les maquettes avec mention explicite. (3) `grep -E "\s"` ne marche pas (GNU grep) — un `color: #111` réel (coche `.diff-case`) avait échappé au premier inventaire.
- **Aucun fichier de formulaire modifié.** Prochaine étape si l'admin valide : appliquer la table aux 3 sources (+ règle des 16px déjà en place), tester Playwright groupes K/R/X/AB, et documenter la recette « formulaires publics » au même niveau que celle des onglets admin (section 2026-06-09).
...
- **`_carteReportEnAttente()`** (index.html, après `_saisonCourante`) : détecte une ligne `inscriptions_cours` `type='carte10'` + `saison=saisonCourante()` + `donnees.isReport`, ET carte non redémarrée (`carte.dateAchat` vide ou < 1er septembre de la saison courante). **Retourne null dans tous les autres cas, y compris sur donnée inattendue (try/catch)** → hors report, l'affichage historique est strictement inchangé (prouvé par AD3 : rendu identique octet pour octet avec détection active vs neutralisée). Avant le 1er septembre, la ligne isReport porte la saison SUIVANTE → détection inactive tout l'été (voulu, cf. maquette).
- **Accueil** : `_carteTrackingHtml` rend `_carteReporteeHtml()` en état reporté — badge « ↩ Reportée de la saison X », cercle N « cours disponibles », 0/N, « Validité — calculée à votre premier cours », encart bleu « Vos N cours vous attendent… pointé en salle ». ⚠️ Couleurs adaptées aux 2 thèmes via variables CSS (écart assumé avec la maquette, qui était en sombre fixe — signalé à l'admin).
- **Onglet Forfait/Carte** : mêmes éléments DOM réutilisés avec valeurs virtuelles (used=0, remaining=N, hasStarted=false → dates/badge masqués, days=null → jamais « Expirée ») ; titre « Ma carte de cours — ↩ reportée de X » ; boutons Renouveler ET Je pointe masqués ; encart injecté (`#carte-report-info`, retiré au re-render hors état reporté) ; barre d'alerte « pensez à renouveler » désactivée.
- **Ceintures** : `renewCarteSelf()` et `ouvrirModalPointerSelf()` refusent (alert + return) en état reporté, même via un chemin résiduel — AD4 vérifie que `eleveData.carte` reste intact.
- **Tests groupe AD** (4) : AD1 état reporté complet ; AD2 retour à la normale après redémarrage ; AD3 non-régression octet pour octet ; AD4 ceintures. ⚠️ Fixtures RELATIVES à la saison d'exécution (piège rencontré : des dates figées « mars/juillet 2026 » faisaient passer la carte pour déjà redémarrée en saison 2025-2026 — la carte « non redémarrée » doit avoir `dateAchat` < 1er sept de la saison courante, ex. printemps de la saison précédente).
- ✅ **VOLET 2 (admin) FAIT** : `pointerCoursAction` détecte `c._fromCoursTango && c.isReport && c.saisonOrigine && !c.datePremierCours` → **chemin de redémarrage dédié** au lieu de `tevPointerCours` (qui échouait en silence : id `inscriptions_cours` introuvable dans `eleves`, catch muet). Redémarrage = résolution du vrai `eleves.id` par email (même cascade que `sauvegarderEditCarte` : carte eleves en mémoire → SELECT par email → upsert), puis fiche remise à neuf : `carte_utilises=n, carte_restants=taille−n, carte_date_achat=date, carte_expiration=calcExpiration(...), carte_statut='Active', carte_paye=true` **et `saison=saisonActive()`** — la bascule de saison fait qu'au rechargement `_buildCartesData` montre la vraie carte eleves (dédup `personnesVues`) au lieu de la pseudo-carte isReport, et l'espace élève repasse en affichage normal (dateAchat ≥ 1er sept). Présences insérées, broadcast `carteUpdate`, notifs CP-E + C1 envoyées, toast « ↩ Carte reportée redémarrée — n/N ». **Les 2 catch de pointage affichent désormais un toast** (fin des échecs muets). Tests **groupe AE** (3, TEV.client intercepté) : AE1 redémarrage complet, AE2 non-régression carte normale → `tevPointerCours`, AE3 échec visible + état local intact.
- ✅ **Compteur `carte_num` remis à 1 en début de saison (2026-08-18)** : il n'était JAMAIS remis à zéro (seulement incrémenté aux 3 chemins de renouvellement) → « 3ᵉ de la saison » affiché à tort sur la 1ʳᵉ carte de la saison suivante. Remise à 1 aux points où une carte démarre une nouvelle saison sur la fiche eleves : (1) redémarrage d'une carte reportée via « + Pointer » (chemin AE) ; (2) redémarrage via « ✏️ Modifier » (`_upd.carte_num=1` seulement si `_isRealReport` — jamais pour l'édition d'une carte ordinaire) ; (3) `_updElevesDI` de l'inscription directe (carte neuve). Les créations de fiche neuves étaient déjà correctes (défaut null → affiché 1). VP (`ignoreDuplicates`) ne touche pas la fiche existante — voulu (groupe M). AE1 vérifie `carte_num:1`.
- 📌 Incohérence sémantique `sauvegarderEditCarte`/`_buildCartesData` sur `reportedRestants` (utilisés vs restants) : devenue sans effet dès qu'un pointage a eu lieu (la pseudo-carte disparaît avec la bascule de saison de la fiche), laissée en l'état.
...
- ✅ **`tevCreerEleve` corrigé aussi (2026-08-18)** : repli figé `'2025-2026'` remplacé par `_tevSaisonCourante()`. **Repli dormant** : l'unique appelant vivant (validation d'inscription, admin.html ~L1977) transmet toujours sa saison. Cache bumpé **`?v=15`**.
- 📌 **Code mort identifié (laissé en place volontairement)** : `renderEleves()` (admin.html ~L8567, bouton « + Nouvel élève » + sous-onglets Attente/Actifs/Inactifs) **n'est plus dans la table d'aiguillage de `doRender`** — écran inaccessible depuis le menu, vestige d'une ancienne version. Son `soumettreEleve` (~L8613) n'appelle donc jamais `creerEleve` sans saison en pratique. Décision admin 2026-08-18 : ne pas supprimer (ne rien casser), juste documenter.
...
- **Encadrés notifications conservés** : les toast / panel 🔔 / push OS des anciennes maquettes sont archivés dans `outils/blocs/` et réinjectés en fin de page par `blocNotifs()`, sous un bandeau orange « rédigé à la main » — les notifications ne passent pas par un gabarit exécutable. Choix admin explicite du 2026-08-17.
- **Cartes (✅ régénérée, 13 cas / 14 emails)** — 2 bugs corrigés au passage : (1) `handleCronFinSaisonC4` utilisait `_c4TokenMap` **jamais déclaré** → `ReferenceError` avalée → email C4 envoyé mais **push OS jamais reçu** (C5 avait bien sa déclaration) ; (2) `handleCronCarteExpiree` (CX) construisait son sujet avec `e.restants`, champ inexistant (colonne `carte_restants`) → **le sujet annonçait toujours « 0 cours non utilisés »** alors que le corps affichait le bon nombre.
- **C3 mis en place (2026-08-17)** — jusque-là, « ↻ Renouveler → **Payé** » appelait `renouvelerCarteAction(..., paye=false, ...)` (le montant n'est saisi qu'ensuite) → **C2b partait**, réclamant un règlement à quelqu'un en train de payer, suivi de C-pay. Désormais : `sansEmailRenouv` supprime C2b **dans ce seul parcours**, et `handleNotifyCartePaiement` accepte `renouvellement:true` → variante **C3** (bandeau « ✓ Votre carte a été renouvelée », encadré « NOUVELLE CARTE DE N COURS », sujet et push adaptés). **C-pay strictement inchangé** sans ce drapeau. Un seul email part.
- **C2 branché (2026-08-17)** — « Renouveler sans payer » de l'espace élève ne prévenait personne (ni email élève, ni panel 🔔). `index.html` appelle maintenant `/api/notify/carte-renouvellement` en `source:'eleve'`, après le renouvellement réel, hors branche démo, en fire-and-forget. Helper `_lienRenouvAC()` (⚠️ logique dupliquée en ligne dans 2 rendus). Tests **groupe AA** (2).
- 📌 **CP-A n'est pas un email** : `handleNotifyCartePonteeAdmin` (« pointée *par* l'admin ») pose la notif in-app de l'ÉLÈVE, met **CP-E** en file pour le lendemain matin et envoie un push à l'élève. L'admin ne reçoit rien. Les anciennes maquettes le présentaient à tort comme un email admin.
- **Essai tango (✅ 14 cas / 23 emails)** — 3ᵉ occurrence du bug `dateLabel` corrigée dans `handleCronEssaiRappelJ7` (E4). **Balayage statique de tout `worker.js` : aucune autre occurrence.** ⚠️ Correction d'une erreur d'analyse : **E-mod EST branché** — l'appel passe par le helper `_notifEssaiEdit` (admin.html ~L4526), pas directement dans `validerEditEssai`. Restent signalés : E7 = alias d'E6, E5b = pas de variante dédiée, et E-cancel dont le sujet dit « inscription modifiée » quand le texte dit « annulée » (**choix admin : laissé tel quel**).
- **Yoga (✅ 12 cas / 20 emails)** — **3 emails réels jamais documentés** ajoutés : `YI-mod` (changement de formule d'un élève régulier), `Y-fin` (cron du 15 juin — ⚠️ **n'envoie rien** si `tev_liens_assoconnect[saison suivante].yoga` est vide), `Y-date` (changement de calendrier). À l'inverse **Y-full et Y2 n'existent pas**. Y-J1a/Y-J1b envoient une **copie admin** non documentée.
- **Cours particuliers (✅ 2 cas / 4 emails)** — 🐛 **corrigé** : les deux emails affichaient les identifiants du formulaire (`jeremy`, `1an`, `passer-cap`) au lieu des libellés. Tables `CP_PROFS` / `CP_NIVEAUX` / `CP_OBJECTIFS` dans `handleNotifyCoursParticulier`, **repli sur la valeur brute** si hors liste. Objet harmonisé (« Jérémy » au lieu de « Braitbart »).
- **Devis (✅ 2 cas / 4 emails)** — 📌 **D1 n'est pas un email** : le bouton « ✉️ Email » ouvre un **brouillon Gmail**, rien ne part automatiquement. D0a/D0b = un seul gabarit, le mode ne change que les champs affichés.
- **CB 3× (✅ 2 cas / 2 emails)** — marqueurs anti-doublon `relance_cb3x_2_sent` / `_3_sent` (à l'inverse de la relance du 22 août, sans marqueur par choix admin).
- **À valider (✅ 7 cas / 12 emails)** — transferts T1-dem/T1-val (+ variante Vincennes), Sorano SR1/SR2, annulation et report faits par l'élève depuis ses emails. 📌 **Les 5 sections « changement de dates » (Paris, Vincennes, Stages, La Dolce Vita, Colectivo) n'existent pas** : ces calendriers ne posent qu'une notification in-app ; **le yoga est le seul à envoyer un email**.
- **`sh outils/regenerer-tout.sh`** régénère les 10 pages d'un coup — à lancer après toute modification d'un email. ⚠️ Les `preview-sources-*.html` restent manuelles : les mettre à jour dans le même commit.
- **Outillage** : `construirePage()` (constructeur de page partagé), `aides` (injecte d'autres fonctions de worker.js, async ou non — `sendBrevoNotification`, `sbFetch`, `authHeaders`, `_emailsInscritsActifs`), `__tables` (autres tables), `__rpc` (réponse de RPC), `opts.args` (handlers à signature non standard comme `(request, url, action, env)`).
- **✅ CAMPAGNE TERMINÉE (2026-08-18)** — les 9 familles sont générées depuis le code, et les 9 pages sources vérifiées (routes, sujets, préheaders, conditions, champs reçus, emails fantômes). Non audité : la mise en forme fine (couleurs, tailles) citée dans certaines sections.
- **Champs « transmis mais jamais lus » — revérifiés un par un** : l'audit précédent en annonçait 27, mais **5 annotations sur 8 étaient fausses** (les champs n'étaient pas transmis du tout). Il en restait **9 réels**, dont un corrigé (`datePremierCours` de C1, désormais affiché). Restent sans usage et sans enjeu : `ville`/`niveau`/`saison` de C1, `lieuDetail` (déjà inclus dans `lieu`) des cours particuliers, `nom` d'I04, `niveau` (doublon) d'I0.
- ⏳ **Deux décisions admin en attente** (sans urgence) : (1) **I17** — le formulaire transmet bien `mode: 'preinscription'`, jamais lu ; un email distinct en mai-août est possible, c'est une question de texte. (2) **`source`** des cours particuliers (`pwa`/`wix`) — pourrait indiquer dans l'email admin d'où vient la demande.
- ⚠️ **Point de vigilance calendaire** : `handleCronYogaFinSaison` (15 juin) **n'envoie rien** si `tev_liens_assoconnect[saison suivante].yoga` est vide — silencieusement. À renseigner avant le 15 juin.
...
- **Ciblage** : `inscriptions_cours?statut=eq.attente_paiement&saison=eq.<saison à venir>`. 🐛 **Corrigé le 2026-08-18 (audit de bascule)** : la formule calculait la saison COURANTE — le 22 août elle aurait donné 2025-2026 et manqué tous les pré-inscrits 2026-2027. Nouvelle formule : `année courante → année+1` (identique tous mois), la doc/preview disait déjà la bonne chose, seul le code était faux. Vérifié en exécutant le handler (banc d'essai) : requête, lien AssoConnect, horaires, livret et Sorano tous sur 2026-2027. ⚠️ Les statuts `demande` (Att. Valid.) sont **exclus** — ces personnes ne peuvent pas encore payer. Fiches sans email ignorées (comptées dans `sansEmail`).
...
- ⚠️ Les emails S-edit/S3/S1 partent avec l'horaire **gravé à l'envoi** depuis les params courants côté client (avec override par date). S1/S1b = corrects par construction (gravé = params du moment de l'inscription). S3/S-edit = envoyés depuis admin.html qui fusionne l'override.
...
- **🐛 BUG S4 CORRIGÉ (2026-07-29) — créneaux absents du rappel J-3** : capture élève → l'encadré affichait le repli générique « Stage Tango & Vous » **sans aucun créneau ni horaire**, et le sujet sans plage horaire. Cause : `handleCronRappelStageJ3` lisait `donnees.stagesDetail`, or pour le **nouveau format** `donnees` = la charge utile complète du formulaire, où les créneaux sont dans **`donnees.inscriptionsParDate[].stagesDetail`** (pas au premier niveau) → `slots=[]` → repli. Seul l'ancien format marchait (normalisé en amont ~L8068). ⚠️ Les fiches **partenaire** du nouveau format ont `donnees: {}` (`stages-pwa.html` L1230) → aucune source JSON. Fix = résolveur `_s4Slots(e, donnees)` à 3 niveaux : (1) entrée de `inscriptionsParDate` correspondant à `targetDate`, (2) `donnees.stagesDetail` (ancien format), (3) repli sur les types gravés dans **`stage_nom`** (« technique|stage1 »), même lecture que l'admin — d'où l'ajout de `stage_nom` aux 2 `select=` de la requête. Thème via `_s4SlotTheme` : gravé, sinon relu dans `_s4StEntry.themes` (`themes[0..3]` = stage1..stage4, cf. `buildSlots`), « Technique Leader & Follower » pour la technique. **Sujet corrigé aussi** : les horaires étant « Xh–Yh », prendre `slots[0].horaire` et `slots[dernier].horaire` bruts aurait donné « 14h30–15h30–17h–18h30 » → on découpe sur « – » pour ne garder que début du 1er et fin du dernier.
- **🐛 BUG S4 (2) CORRIGÉ (2026-07-29) — créneaux dans le désordre** : révélé par les vraies données du 1/08/2026 (Initiation **14h–15h30** = stage1, Technique **15h30–16h30**, Séquence complexe **16h30–18h** = stage2). `buildSlots` (stages-pwa.html L668-673) construit les créneaux dans un **ordre FIXE** — stage4, stage3, **technique**, stage1, stage2 — qui n'est chronologique que si la technique est le 1er créneau de l'après-midi (cas des horaires de saison : tech 14h, s1 15h, s2 16h30). Une date **personnalisée** peut la placer au milieu → l'email listait « 15h30 Technique » AVANT « 14h Initiation », et le sujet annonçait **15h30–18h** au lieu de 14h–18h. Fix : helper `_s4Min('14h30')→870` + `.sort()` chronologique sur les créneaux avant rendu ET avant calcul du sujet — **le worker reproduit ainsi ce que fait déjà le formulaire public**. ✅ **`buildDateEntry` (stages-pwa.html) trie DÉJÀ ses créneaux — L674 `items.sort((a,b)=>(a.horaire||'').localeCompare(b.horaire||''))`** : le formulaire public était donc correct, seul le worker ne triait pas. (Le worker trie en minutes via `_s4Min` plutôt qu'en `localeCompare` de chaîne, plus robuste : « 9h » vs « 10h » se compare mal en texte.)
- 🐛 **BUG S4 (3) — montant absent + encadré divergent (2026-07-29)** : le rappel listait les créneaux mais **sans le prix ni la mention « prévoir l'appoint »**, contrairement à S1/S3. **Cause de fond** : S1 possède un constructeur d'encadré complet (`buildStageBox` ~L7783) alors que **S4 avait le sien, réécrit à la main et amputé** — d'où une série de correctifs au coup par coup (créneaux, horaires, thèmes, ordre, prix). Fix : `stageBox` de S4 **réaligné à l'identique sur `buildStageBox`** (même mise en page, même bloc Lieu, même bandeau « Total à régler sur place », mention de l'appoint toujours affichée). Montant = colonne **`total_inscrit`** de la fiche (posée à l'inscription, recalculée par l'admin en cas de modification des créneaux ~L11840) → montant propre à chaque personne, partenaire inclus ; ajoutée aux 2 `select=`.
- 📌 **RÈGLE : S4 et S1 doivent rester jumeaux.** Les deux emails sont quasi identiques (seuls diffèrent le bandeau, l'intro, le bouton 👍 et la note de rappel) — toute évolution de `buildStageBox` (S1) doit être répercutée dans `stageBox` de `handleCronRappelStageJ3` (S4), et réciproquement. Idéalement, factoriser un jour les deux en un seul helper partagé.
- 📌 **Règle : le worker doit reconstruire les créneaux COMME `buildDateEntry`** — quels créneaux (depuis l'inscription) + horaires/thèmes relus dans les params (override par date → saison) + tri chronologique. Ne pas se contenter des valeurs gravées dans `donnees`.
- **✅ Correctif S4 worker (rappel J-3, 2026-07-24)** : le rappel J-3 (`handleCronRappelStageJ3`, cron) est le SEUL email qui peut partir longtemps après l'inscription → il lisait l'horaire **gravé** (`donnees.stagesDetail[].horaire`), donc un horaire personnalisé APRÈS l'inscription n'était pas répercuté. Fix : S4 **relit maintenant les horaires depuis les params** (`tev_params_stages_<sai>.horaires` + override `st.horaires` de la date, fusion override prioritaire) — exactement comme il le faisait déjà pour l'adresse. Helper `_s4SlotHoraire(sl)` : map `type`→clés (`technique`→`tech`, `stage1..4`→`s1..s4`), repli sur `sl.horaire` gravé si params absents (aucune régression). Remap des slots en tête de boucle → affichage ET sujet utilisent l'horaire actuel. **⚠️ Changement de règle métier assumé pour S4 uniquement** : un changement d'horaire tardif est désormais répercuté dans le rappel (c'est le but : l'élève reçoit l'horaire vrai du jour J). Non testable en runtime (worker) — `node --check` OK, pattern miroir de l'adresse déjà éprouvé.
...
2. **Fenêtre passée de 7 jours** (correction demandée le jour même : « il faut absolument pouvoir pointer dans les jours qui suivent ») : les dates de J-7 à aujourd'hui avec inscrits restent visibles et pointables ✓/✗ ; au-delà → onglet 📋 Historique. Résout aussi l'éviction : le plafond `slice(0,20)` ne compte plus le vieux passé.
...
- **✅ FAIT — « Pointage »** (2026-07-24) : essais validés ordonnés via `_essaiOrdonnerGroupes(_groupCouples(conf), …)` ; bloc **Élèves ★ présents déplacé AVANT la liste d'attente** (3bis = élèves ★ absents juste après) ; la section « ⏳ Liste d'attente » (attenteGrp, tous statut attente) passe dans le même helper → couples att. → guideurs att. → guidées att. (exp croissante) ; Supprimés inchangés tout en bas.
- **✅ FAIT — Impression 🖨** (`imprimerEssaiTango`) : `g.conf` et `g.att` aplatis via helper `_aplatir` (même ordre que les vues), Élèves ★ entre les deux avec **alternance guideur/guidée** (avant : ordre brut). ⚠️ **Changement assumé** : les fiches supprimées sont désormais **exclues de l'impression** (avant le fix, faute de filtre statut, elles étaient imprimées comme des essais normaux — piège sur feuille papier).
- **Tests groupe S** (`tests/s-essai-ordre.spec.js`, 6 tests) : S1 ordre des 5 catégories + alternance + vide=0 ; S2 non-mutation + stabilité ; S3 ordre DOM Par date + supprimés en bas ; S4 boutons ciblés par id après ré-ordre ; S5 ordre DOM Pointage complet (validés → ★ → ★ absents → attentes → supprimés) ; S6 impression (ordre + supprimés exclus). ⚠️ `renderTab()` est ASYNCHRONE (spinner puis `setTimeout(doRender, 40)`) → tout test qui lit le DOM après `renderTab()` doit `waitForFunction` sur les `.point-row`. Suite complète : 79/79.
...
- [ ] **Bouton « ✉️ Renvoyer l'email » : ne plus notifier l'admin** (demandé 2026-09-04, après test réel — le renvoi fonctionne, mais il déclenche aussi le push et l'email admin « nouvelle demande », inutiles pour un renvoi et qui **polluent le repère du panel 🔔** servant à retrouver les fiches restées sans email). **Principe** : `renvoyerEmailInscription` transmet un drapeau (`renvoi: true`) que `handleNotifyInscriptionCours` lit pour sauter les 3 envois destinés à l'admin — email admin (`await sendBrevo(adminEmail, …)` ~worker.js L5327), notification du panel (`_insertNotification` ~L5336) et push (`getFcmTokensAdmin` + `sendFcmPush` ~L5341). Ces 3 blocs sont déjà isolés ; l'email élève et tout le reste ne bougent pas. **Même patron que `sansEmailRenouv` (cartes) et la case « Notifier les élèves » (vidéos) : sans drapeau, comportement historique à l'identique.** ⚠️ **Test de non-régression obligatoire** : une charge utile SANS drapeau doit continuer à produire les 3 envois — c'est le seul vrai risque (qu'une inscription réelle cesse de notifier l'admin). **2 décisions à prendre avant de coder** : (1) garder ou non une notification discrète « ✉️ Email renvoyé à X » dans le panel 🔔 (sans email ni push) pour l'historique des renvois ; (2) vérifier si le bouton voisin **« 🎓 Relancer justif »** notifie lui aussi l'admin à chaque relance — si oui, traiter les deux pareil plutôt que d'avoir deux comportements voisins divergents.

- [ ] **Push admin manqué isolé (2026-07-18) — surveiller, ne rien coder pour l'instant** : une demande de cours particulier a produit email admin + notif panel 🔔 mais PAS de push (l'autre demande CP du même jour → push OK ; push test OK ; pas de mode Concentration). Diagnostic : échec ponctuel de l'étape push dans `handleNotifyCoursParticulier` (worker.js ~8573 — `getFcmTokensAdmin` ou `sendFcmPush` a raté une fois, erreur avalée par le try/catch volontaire, pas de retry, pas de logs Cloudflare rétroactifs). Décision utilisateur : **attendre** — les notifications critiques sont doublées email+panel. **Si ça se reproduit plusieurs fois → ajouter un retry (1 nouvelle tentative avec petite pause) sur l'envoi du push admin dans les handlers de notification.**
- [ ] **Email automatique « veille d'essai » aux listes d'attente essai tango, décliné par RAISON** (demandé 2026-09-02 — à cadrer avec l'admin avant de coder) : automatiser l'email envoyé à la main le 02/09 (gabarit une-fois `email-attente-essai-3sept.html`, avec bouton de copie riche pour Gmail — servira de maquette de départ). Cible : les personnes encore `statut='attente'` d'un cours d'essai imminent. **Variantes à distinguer selon la raison de l'attente** (déterminable au moment de l'envoi via la fiche — rôle, seul·e/couple — et les quotas réels de la date via `compter_inscrits_essai`) :
  1. **En attente car cours COMPLET** — 3 sous-cas : couple mis en attente (quota atteint sur un des rôles), guideur seul refusé (complet guideurs 22/22), guidée refusée (complet guidées 23/23) ;
  2. **En attente car pas assez de guideurs** (guidée seule, parité — le cas historique E2, celui du 02/09).
  Contenu par variante : raison honnête + propositions adaptées — choisir une autre date en **répondant à l'email** (dates dynamiques : prochaines dates du même cours, jamais figées) et, quand ça débloque, venir à deux avec une personne du **rôle manquant** (guideur pour une guidée en manque de guideurs, etc. — pas pertinent pour un couple ni si le cours est complet des deux côtés). À trancher au cadrage : déclencheur (cron J-1 ? J-2 ?), interaction avec la validation manuelle de dernière minute (mention « nous vous recontacterons si une place se libère »), anti-doublon (marqueur pour ne pas renvoyer si l'admin a déjà agi), et mise à jour previews+sources (nouvelle famille d'emails).

- [ ] **Vue Archives : liste blanche des boutons inoffensifs** (diagnostiqué 2026-09-01, validé dans le principe par l'admin — liste à confirmer avant de coder) : en saison archivée, le mode lecture seule (`.saison-archivee`, admin.html CSS ~L119-135 + wrapper dans `doRender` ~L6103) masque TOUT `.btn`/`.btn-point`/`.btn-row` et désactive tous les inputs. Trop large : il masque aussi (1) les **sous-onglets de Cartes 10** « Pointage/Détails/Supprimées » (des `<button class="btn" data-action="cartes-subtab">`, contrairement aux `.sub-tab` des autres rubriques qui survivent) → impossible d'atteindre Détails ; (2) les boutons de **consultation pure** (🖨 Imprimer/PDF, 📋 Copier les emails, 📇 Contacts, 🕐 Historique) ; (3) la **barre de recherche** (pointer-events:none sur tous les inputs). **Fix prévu** : garder le masquage général (garde-fou intact) + classe « autorisé en archive » posée UNIQUEMENT sur les éléments inoffensifs listés + règle réactivant `input[type=search]`. Liste blanche proposée : navigation Cartes 10, Imprimer, Copier emails, Contacts, Historique, recherche. 📌 Au passage : les règles CSS `.saison-archivee` sont dupliquées (L119-122 = L132-135). 📌 Vu aussi le 2026-09-01 : `updateSaisonSelect()` n'est appelé qu'au boot (pas après `chargerDonnees`) — la liste Archives s'est remplie chez l'admin, mais le timing est fragile ; à re-remplir après le 1er chargement des données dans la même passe.
...
- [ ] **Mettre en avant l'appli élèves sur le site Wix** (2026-07-24 · **relancé 2026-08-28** — fichier renvoyé à l'admin, prêt à coller ; reste côté admin : coller les 4 blocs dans Wix + 3-4 captures WebP ≤ 100 Ko pour la page dédiée ; Claude peut générer les captures sur demande) : textes prêts à coller dans **`contenus/wix-appli-eleves.md`** — (1) bande page d'accueil « Bien plus qu'un cours de tango » (4 bénéfices : vidéos 🎥 / carte 💳 / notifs 🔔 / communauté 💬, CTA = cours d'essai) ; (2) encadré sur les pages Cours Paris/Vincennes ; (3) ligne sur la page cours d'essai ; (4) page dédiée « L'appli élèves » (menu École, 5 sections H2 + CTA). **Règles** : jamais de lien public direct vers app.tangoetvous.fr (réservé aux inscrits, CTA toujours = cours d'essai) ; anti-alourdissement SEO : 1 image/section WebP ≤ 100 Ko, aucune vidéo intégrée, aucun widget Wix, vérifier Signaux Web essentiels après mise en ligne. Hors site : posts Google Business (3 fiches) + mention orale au cours d'essai.
...
- [ ] **Environnement staging** : créer un environnement de test (2ème Worker Cloudflare + 2ème projet Supabase) pointant sur le même repo GitHub, branche `staging`. Workflow GitHub Actions dédié : push sur `staging` → déploie sur le Worker de test. Permet de valider les nouveautés avant de les merger sur la branche de production `claude/dance-school-app-RTqb5`. Demi-journée de travail.
...
- [ ] **Mettre à jour les actions GitHub vers Node.js 24** : ✅ `backup-csv.yml` et `keep-alive.yml` déjà en `@v5` (re-vérifié 2026-09-01 dans les fichiers). **Reste uniquement** `deploy.yml` → `actions/checkout@v4` → `@v5`. **Décision admin 2026-09-01 : ATTENDRE — pas pendant la rentrée**, même à risque faible ; à faire plus tard dans la saison (protocole convenu : changer, pousser, vérifier le run de déploiement vert avant de confirmer). Sans urgence : Node 20 encore supporté plusieurs mois ; signal = warning jaune "Node.js 20 deprecated" dans un run Actions. Risque de casse quasi nul (checkout v5 = simple bump runtime ; si échec, l'étape `wrangler deploy` ne s'exécute pas → prod intacte). Test = push → run deploy.yml vert/rouge dans GitHub Actions.
...
**⚠️ Fix 2026-07-17 — synchro Supabase des dates : saison courante ET suivante.** Le formulaire affiche les stages de la saison courante ET de la saison suivante (fenêtre `_isEte` en juillet-août, `chargerDonnees` lit toutes les clés `tev_dates_stages_*` du localStorage). MAIS le `DOMContentLoaded` (~ligne 1400) ne resynchronisait depuis Supabase QUE `tev_dates_stages_<saisonCourante()>` — jamais la saison suivante. Conséquence : en été, les prochains stages (saison N+1) restaient figés sur le **cache local de chaque appareil** → dates différentes selon le device (Mac vieux cache = 1 date, téléphone = 3), et **un nouveau visiteur pouvait voir 0 stage** (le fallback `DATES_STAGES_RAW` codé en dur était périmé). Corrigé : la synchro boucle maintenant sur `[saisonCourante, saisonSuivante]` (`_saiSuiv = (year0+1)+'-'+(year1+1)`) et réécrit les deux clés localStorage depuis `TEV.getParam`. **Règle** : toute donnée stages affichée sur les deux saisons doit être resynchronisée pour LES DEUX saisons, pas seulement la courante. Validé Playwright groupe K (chargement stages-pwa sans erreur JS).

**⚠️ Fix 2026-07-17 (suite) — synchro AUSSI des paramètres `tev_params_stages_<sai>` (horaires/tarifs/adresse), deux saisons.** Après le fix des dates, les HORAIRES affichés restaient faux : le `DOMContentLoaded` ne synchronisait que `tev_dates_stages_*`, jamais `tev_params_stages_*` (qui contient les horaires globaux). Or `chargerDonnees` lit `globalHor = pp.horaires` depuis `localStorage['tev_params_stages_<sai>']` (ligne ~697, et re-lit la saison du 1er stage ligne ~725) → sans synchro, `globalHor` vide → l'horaire retombait sur `DEFAULTS_HORAIRES_STAGES` au lieu des valeurs admin. **Vérifié via sous-agent Explore (structure confirmée PLATE des deux côtés, aucun mismatch)** : `admin.html:sauverHorairesType` (~5261-5280) sauve `tev_params_stages_<sai>.horaires` à plat `{tech_deb,tech_fin,s1_deb,…}` (+ `TEV.setParam` Supabase), et `stages-pwa.html buildDateEntry` lit `globalHor['tech_deb']` — même schéma plat. Les overrides par date (`st.horaires`, via `sauverHorairesDate` ~5053) sont aussi plats et voyagent dans `tev_dates_stages_<sai>` (déjà synchro). Fix : la boucle de synchro récupère et réécrit AUSSI `tev_params_stages_<s>` pour les deux saisons (horaires + tarifs + adresse d'un coup). Validé Playwright groupe K (7/7). **Règle permanente** : le formulaire stages lit horaires/tarifs/adresse depuis `tev_params_stages_<sai>` en localStorage seulement — toute nouvelle donnée param affichée doit être ajoutée à cette boucle de synchro Supabase (les deux saisons).

**⚠️ Fix 2026-07-17 (final) — CAUSE RACINE : les horaires de la saison du 1ER stage s'appliquaient à TOUTES les dates.** Même avec la synchro corrigée, les stages 2026-2027 affichaient de mauvais horaires. Diagnostic (pop-ups console utilisateur + indice décisif : « ça semble être les infos pour la date du 1/08/26 ») : en été la liste des 3 prochains stages MÉLANGE deux saisons — le 1er août 2026 appartient à la saison **2025-2026** (saison = sept→août) et arrive en tête, suivi des dates de sept/oct **2026-2027**. Or `chargerDonnees` calculait `sSai` depuis `stages[0]` uniquement : 1er août → sSai=2025-2026=sai → la branche qui charge `tev_params_stages_2026-2027` ne s'exécutait JAMAIS → `globalHor` (saison courante) appliqué à toutes les dates. Fix : helpers `_saisonDe(date)` + `_paramsDe(saison)` (cache) — `DATES_STAGES=stages.map(st => buildDateEntry(st, _paramsDe(_saisonDe(st.date)).hor, ...))` : **chaque stage utilise les horaires/adresse de SA saison** (repli sur la saison courante si params absents). `TARIFS_STAGES` reste basé sur la saison du 1er stage (bloc tarifs unique, comportement historique). Tests : **R3** (`tests/r-stages-horaires.spec.js`) reproduit le scénario exact (1er août avec horaires per-date + 19 sept/3 oct saison suivante, chaque date vérifie ses horaires) ; R1/R2/K toujours verts. **Règle** : toute logique « paramètre par saison » appliquée à une LISTE multi-saisons doit résoudre la saison PAR ÉLÉMENT, jamais depuis le premier élément.
...
- **2ᵉ passe — blocs, conditions, origines de données (exhaustive)** : préheaders comparés handler par handler (5 écarts corrigés : T1-dem partage en réalité le préheader de T1-val, CX, C2, E-admin-cancel, Y1) ; **7 conditions** documentées (`si vincennes`, `si livretUrl`, `si couple`, `≤3 j`…) toutes vérifiées présentes dans leur handler — **0 anomalie** ; **168 champs `Body`** confrontés au code → **27 transmis mais jamais lus par le worker**, annotés section par section plutôt que supprimés (ils sont bien envoyés par l'admin — vérifié pour `carte-bienvenue` — mais n'apparaissent pas dans l'email).
- ✅ **Portée finale** : sujets, routes, préheaders, conditions et champs `Body` vérifiés **exhaustivement** contre `worker.js`. Restent non audités : la mise en forme fine (couleurs, tailles) décrite dans certaines sections.
