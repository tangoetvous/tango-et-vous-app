...
## À faire / en suspens
- [ ] **Cartes de 20 cours** : définir comment les traiter dans l'appli (actuellement seules les cartes de 10 cours sont gérées). Questions à trancher : même affichage que les cartes 10 avec un compteur /20 ? Même algorithme d'expiration (3 mois + gaps) ? Même workflow de renouvellement ? À discuter avant d'implémenter.
...
- [ ] **Stages — créneaux entièrement paramétrables** : actuellement plusieurs valeurs sont hardcodées (technique 14h–15h, 2 stages max, horaires par défaut dans `DEFAULTS_HORAIRES_STAGES`). Objectif : jusqu'à 5 créneaux par journée de stage (technique + jusqu'à 4 stages ou toute combinaison), créneaux et thèmes entièrement personnalisables par date dans Paramètres → Stages, zéro valeur hardcodée — tout vient des paramètres. Implique de revoir le modèle de données des dates de stage (structure `slots[]` par date plutôt que champs `s1/s2/s3/s4/tech` fixes), le formulaire d'inscription `stages-pwa.html`, l'affichage dans l'espace élève (`index.html` → `renderStages()`), et le générateur de paramètres dans `admin.html`.
