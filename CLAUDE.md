...
| **E-J1a** | Admin clique ✓ **Présent** dans le Pointage (`pointerEssai present=true`) · **pas de cron** | `confirme` | Élève | Bandeau vert ✓ · référence explicite au cours effectué (ville, niveau, date, horaire depuis params) · process inscription (demande → validation → paiement CB 1x/3x, chèque, espèces) · encadré parité pour les solos · "les cours continuent dès la semaine prochaine" · bouton "Demande d'inscription →" (AssoConnect) · **pas de lien vers un autre cours d'essai** (une personne ne peut faire l'essai qu'une fois) |
| **E-J1b** | Admin clique 🚫 **Absent** dans le Pointage (`pointerEssai present=false`) · **pas de cron** · ne s'applique pas aux élèves réguliers du cours | `confirme` | Élève | Bandeau orange 💙 "Vous nous avez manqué" · référence au cours manqué (ville, niveau, date) · bouton "↩ Choisir une nouvelle date" (formulaire cours d'essai — car pas encore venu, l'essai n'a pas eu lieu) · note pas de pénalité |
