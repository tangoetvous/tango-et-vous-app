...
- **Chronologie** : `main` existait le 05/04 (les sessions de juin/juillet portent déjà `refs/heads/claude/dance-school-app-RTqb5` → le basculement de branche par défaut a eu lieu entre avril et juin). GitHub interdisant de supprimer la branche par défaut, la suppression de `main` a forcément suivi ce basculement. Aucune trace dans `CLAUDE.md` ni `HISTORIQUE.md` — d'où cinq mois de bombe à retardement. Vestige encore visible : l'item « Environnement staging » de la todolist parle de « merger sur `main` (prod) ».
- 📌 **Règle** : avant de supprimer une branche, vérifier qu'aucune session n'y est adossée — et consigner tout changement de branche par défaut ici.
- **Rien n'a été perdu** (le travail était poussé, la mémoire du projet est dans ce fichier). Recréer `main` sur le même commit reste possible si l'on veut tenter de rouvrir l'ancienne session — non fait à ce jour, sans urgence (elle était à 96 % de sa capacité de contexte).
