...
- [ ] **Cartes — compteur `carte_num` + historique consultable** (validé 2026-07-02) : (1) compteur `carte_num` (1ère/2ème/3ème… carte de la saison) incrémenté à CHAQUE chemin de renouvellement (élève "sans payer", admin modal Renouveler) — clé qui permet de découper l'historique par carte ; (2) affichage du numéro de carte dans l'accueil élève et dans Cartes 10 admin ; (3) historique des cours pointés par carte (données déjà présentes : table `presences`, jamais effacée au renouvellement ; l'élève les reçoit déjà via `tevGetEleve`) — admin : bouton "📜 Historique" sur chaque fiche de Cartes 10 → Détails ; élève : lien "Voir l'historique de mes cours" dans la section carte de l'accueil. Lecture seule → risque très faible. Pour les cartes déjà renouvelées : initialisation manuelle ou reconstruction approximative (total présences ÷ 10).
- [ ] **Accueil élève — un encadré par cours de la saison** (validé 2026-07-02) : pour chaque inscription active de la saison courante (`_inscrActives`), un encadré avec nom du cours, jour, horaire, type de formule (forfait annuel / carte) + compteur et date d'expiration si carte. Toutes les données sont déjà chargées au login (inscriptions, horaires via `_coursLine`, carte via `eleveData.carte`) — pur affichage, risque faible. ⚠️ Points de design à trancher avant : (1) la carte 10 est PAR PERSONNE (commune aux 2 cours) — décider comment l'afficher sur 2 encadrés sans laisser croire à 2 cartes distinctes ; (2) redondance avec les lignes "Tes cours" du hero et la section "Ma carte de 10 cours" — remplacer ou cumuler ?
- [ ] **Pointer une carte expirée avec cours restants (exception admin)** : actuellement impossible de pointer un élève dont la carte est expirée même s'il lui reste des cours non utilisés. Ajouter un bouton "Exception — pointer quand même" dans Cartes 10 → Détails et/ou dans Cartes 10 → Pointage, visible uniquement quand `carte_expiration < aujourd'hui && carte_restants > 0`. Le pointage s'effectue normalement mais sans vérification de la date d'expiration.
...
- [ ] **Tarif réduit dans "Valider Paiement"** (Inscriptions Tango) : (1) bouton toggle "Tarif réduit" dans le formulaire VP ; (2) stocke `donnees.tarifReduit=true` dans `inscriptions_cours` à la soumission ; (3) envoie un second email générique (en plus de I03) à l'élève lui demandant d'envoyer un justificatif à `regardsepose@gmail.com` — via nouvelle route `POST /api/notify/justificatif-tarif-reduit` dans `worker.js`, email depuis `contact@tangoetvous.fr` ; (4) pastille rouge cliquable **"R"** sur la fiche dans **Élèves Tango** tant que `donnees.justifRecu` n'est pas `true` ; (5) clic sur la pastille → PATCH `inscriptions_cours` pour poser `donnees.justifRecu=true` → pastille disparaît. Risque faible (tout additif, `donnees` JSON ignore les champs inconnus, nouvelle route worker indépendante).
...
- [ ] **Articles tango — Publications** : rédiger les articles tango à diffuser dans l'espace élève (onglet Publications) et les programmer. **Rythme : 1 article par semaine, début octobre → fin juin** (~39 articles par saison). À faire avec l'utilisateur : choix des sujets, rédaction, dates de publication. **Saisie complète prévue avant fin août 2026** pour que la diffusion roule seule toute la saison. Lié à l'item "Site éditorial tangoetvous.fr" ci-dessous (double diffusion appli + site).
- [ ] **Site éditorial SEO sur tangoetvous.fr** (validé 2026-07-05) : contenu complémentaire au site Wix (JAMAIS de copie — duplicate content pénalisé), qui renvoie vers www.tangoetvous.com. Objectif : première page Google sur les requêtes long-tail. **3 piliers** : (1) **Magazine tango** — les 39 articles publiés AUSSI en public (double canal, un seul effort de rédaction) ; (2) **Lexique du tango argentin** — réutiliser les 108 termes du curriculum "Mon niveau" (ocho, sacada, boleo…), une page/section par terme, requêtes type "sacada tango définition" ; (3) **2-3 pages locales** — "cours tango Vincennes", "tango Est parisien" (requêtes locales peu concurrentielles = 1ère page atteignable). **Saisie admin (spec 2026-07-05)** : section dédiée **ARTICLES** dans Paramètres (distincte de l'onglet Publications existant) avec les champs : Titre, Sous-titre, Contenu, **Vidéos (plusieurs possibles, upload Cloudinary)**, Photo (upload Cloudinary), + date de publication programmée. **Mise en page soignée** de chaque article (template magazine : photo d'en-tête, titre/sous-titre élégants, vidéos intégrées) — appliquée dans l'espace élève ET sur le site public. **Pipeline automatique** : case à cocher "Publier aussi sur tangoetvous.fr" sur chaque article programmé (flag dans `donnees`, ex `sitePublic:true`) ; le Worker Cloudflare sert le domaine racine tangoetvous.fr avec rendu HTML **côté serveur** depuis Supabase `publications` (lecture seule, SEO-friendly : vraies balises title/meta/h1) + sitemap.xml auto. Chaque article se termine par un lien naturel vers le site Wix (cours d'essai). Config Cloudflare : ajouter le custom domain `tangoetvous.fr` au Worker. Attentes réalistes : 3-6 mois avant les premiers résultats, 12 mois pour de vraies positions — le rythme hebdomadaire compte plus que le volume.
