...
- [ ] **Flux iCalendar (ICS) — abonnement calendrier téléphone** : route Cloudflare Worker `/calendar/:token.ics` qui génère un fichier ICS dynamique depuis Supabase. Cours réguliers = `RRULE:FREQ=WEEKLY` (jeudi Paris / lundi Vincennes) avec `EXDATE` pour les vacances/exceptions. Milongas + stages inclus. Token = hash de l'email → URL personnalisée par élève. Bouton "Ajouter à mon calendrier" dans l'espace élève (onglet Agenda) + même bouton dans l'admin. Changements admin → répercutés à la prochaine synchro du téléphone (quelques heures iOS, jusqu'à 24h Google). Complémentaire aux push notifications (ICS = planning de fond, push = alertes immédiates). Durée estimée : ~demi-journée.
