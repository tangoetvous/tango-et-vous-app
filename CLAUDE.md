...
- **RESTE À FAIRE** : (1) **rubrique élève** (index.html) : liste par cours (saison courante), recherche, lecteur lecture seule, bouton « Proposer une vidéo » (upload TUS + notif `video-a-valider`) ; (2) **file modération** admin (À valider → approuver [notif `video-publiee`] / refuser) ; (3) **suppression Bunny** à « refuser/supprimer » (worker : `DELETE https://video.bunnycdn.com/library/701214/videos/{id}` avec AccessKey) + bouton supprimer biblio ; (4) tests Playwright élève.
