...
## À faire / en suspens
- [x] **Inscriptions Tango → Supprimés — actions manquantes** : bouton ↩ Rétablir (restaure le statut original `attente_paiement` ou `demande` via `statut_avant_suppression`) + bouton 🗑 Définitif (DELETE réel avec modale de confirmation). Colonne `statut_avant_suppression TEXT` ajoutée sur `inscriptions_cours` (SQL exécuté 2026-05-27). Fonctions `retablirCoursInscr` + `supprDefCoursInscr` dans `admin.html`. ✅ 2026-05-27
- [x] **Remplacer les iframes Wix par des liens directs** : formulaires accessibles via liens directs, bouton "← Retour au site" → `tangoetvous.com` après soumission. ✅ Fait — **Vérifié 2026-05-28** : les 6 formulaires publics ont tous un bouton doré "← Retour au site Tango & Vous" avec `target="_top"` (navigue au niveau de la page Wix entière, pas de l'iframe) → `https://www.tangoetvous.com` sur l'écran de succès : `inscription-cours.html` ✅ · `cours-essai.html` ✅ · `essai-yoga.html` ✅ · `stages-pwa.html` ✅ · `cours-particuliers.html` ✅ · `demande-devis.html` ✅
- [x] **Emails automatiques + notifications — à construire ensemble** — FAIT (session 2026-05-21) : voir ligne ci-dessous.
...
- [x] **Notifications stages + milongas** — FAIT (session 2026-05-15) : publications auto-générées depuis Paramètres (stages J-20/J-14/J-7, milongas J-14/J-3) + sync automatique à chaque modif + cron S4 rappel J-3 par email + push élèves.
...
- [ ] **Activer sauvegardes Supabase** : nécessite le plan Pro (25$/mois) — **non fait, plan payant**. Backup CSV nocturne via GitHub Actions `backup-csv.yml` utilisé à la place (suffisant).
...
- [x] **Mettre à jour les actions GitHub vers Node.js 24** — FAIT 2026-05-28 : `actions/checkout@v4` → `@v5`, `actions/upload-artifact@v4` → `@v5`, `dawidd6/action-send-mail@v3` → `@v5` dans `backup-csv.yml` et `keep-alive.yml`.
- [ ] **Backup "coffre-fort numérique" — sécurité contre piratage/sinistre** : **priorité immédiate (gratuit, 5 min chacun)** : activer la 2FA sur GitHub, Cloudflare et Supabase — c'est la vraie protection contre la suppression malveillante. En cas de sinistre complet : (1) **Code** → redéployé en ~2 min depuis GitHub vers Cloudflare, rien à faire de plus ; (2) **Données** → `supabase/schema.sql` (schéma complet) + CSVs nightly déjà envoyés par email → restoration ~2-3h dans un nouveau projet Supabase, **sans besoin de tout réexpliquer à Claude** (schéma + logique métier documentés dans `supabase/schema.sql` + `CLAUDE.md`). **Backup local optionnel** : télécharger périodiquement le repo GitHub en ZIP (`Code → Download ZIP`) → stocker dans Google Drive (gratuit) — complète les artifacts GitHub Actions (90 jours de rétention).
...
- [x] Installer l'appli sur Mac (PWA) — ✅ confirmé
...
- [x] **Nettoyage tokens FCM invalides** — déjà implémenté dans `sendFcmPush` (worker.js) : supprime les tokens `NOT_FOUND`/`UNREGISTERED` FCM après chaque envoi. `sendWebPush` gère déjà les 410/404 Web Push (iPhone/Safari). ✅
- [x] **Notifications + emails lors des modifications ✏️ essai** — FAIT 2026-05-28 : E-mod tango (`validerEditEssai` → `/api/notify/essai-modifie`) + Y-mod yoga (`validerEditEssaiYoga` → `/api/notify/essai-yoga-modifie`). Email Brevo avec ancienne date barrée + nouvelle date en vert. Fix sender Brevo yoga : `tangoetvous@gmail.com` + `replyTo: regardsepose@gmail.com`.
- [x] **Étendre 🔔 + push aux événements** — VÉRIFIÉ 2026-05-28 : tous les 7 événements sont déjà câblés avec panel 🔔 (`_insertNotification`) + push OS admin (`getFcmTokensAdmin` + `sendFcmPush`) : essai tango (`/api/notify/inscription-essai`), essai yoga (`/api/notify/inscription-essai-yoga`), demande d'inscription tango (`/api/notify/inscription-cours`), inscription stage (`/api/notify/inscription-stage` — push élève aussi par date), cours particuliers (`/api/notify/cours-particulier`), demande de devis (`/admin/api/devis`), RSVP milonga (`/api/notify/milonga-rsvp`). Tous avec `await` + `try/catch` (pas de fire-and-forget).
- [x] **Push élève — pas de cours la semaine prochaine** — FAIT 2026-05-28 : handler `handleCronPasDeCours` dans `worker.js` + workflow `.github/workflows/pas-de-cours.yml` (deux jobs : `pas-de-cours-paris` cron vendredi 9h Paris, `pas-de-cours-vincennes` cron mardi 9h Paris). Vérifie si hier était un jour de cours → si le prochain cours est à plus de 7 jours → push OS + notification in-app à tous les élèves `statut='inscrit'` de la ville concernée.
